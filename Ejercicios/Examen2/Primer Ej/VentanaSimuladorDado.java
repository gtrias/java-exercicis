...
public class VentanaSimuladorDado extends Frame{
	private TextField tTiradas,tfVeces1,tfVeces2,tfVeces3,tfVeces4,tfVeces5,tfVeces6;
	private Checkbox cManual,cAutomatico;
	private Button bSimular, bLimpiar, bSalir;
	private TextArea taResultados;
	private CheckboxGroup grupoModo;
	private int [] dado = {0,0,0,0,0,0};
	...
	public void simulacion(int veces){
		...
		for(int i=0; i<veces; i++){
			...
			taResultados.append(" "+resultado+1);
			actualizarText();
		}
		...
		public void actionPerformed(ActionEvent e){
			if(e.getSource() == bSimular){
				if(grupoModo.getSelectedCheckbox() == cManual)
					simulacion(1);
				/*else
					String tiradas = tTiradas.getText();
					tiradas.
